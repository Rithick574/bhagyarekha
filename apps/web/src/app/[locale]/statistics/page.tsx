...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: getMessages(locale).placeholder.statisticsTitle };
}
    ...
    <div className="space-y-4">
      <section className="card px-5 py-6 sm:px-7" aria-labelledby="page-title">
        <h1 id="page-title" className="text-[1.75rem] font-bold leading-tight">
          {messages.placeholder.statisticsTitle}
        </h1>
        <p className="mt-3 text-ink-secondary">{messages.placeholder.statisticsBody}</p>
